...
### حماية الـ API Endpoint (اختياري)

لو `API_KEY` متعين في ملف `.env` على السيرفر، كل الطلبات تحتاج header المصادقة:

```
Authorization: Bearer your-api-key
```

> **ملاحظة مهمة:** `API_KEY` هو حماية اختيارية لسيرفرك (زي password). لو مش متعين في `.env`، الـ API يشتغل بدون أي مصادقة (زي التشغيل المحلي). ده **مختلف تماماً** عن مصادقة Claude أدناه.

### طرق المصادقة الخلفية (Backend Authentication)

الخادم يدعم عدة طرق للمصادقة مع Claude (ده بيتعمل على السيرفر تلقائياً، مش من العميل):
